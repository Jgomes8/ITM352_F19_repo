...
//EX2A--------------------------------------------------------------
//NEW INFO----------------------------------------------------------
app.get('/use_session', function (request,response) { //NOTE: we don't have to set a session. If you come to the server without you being identified or not having a session ID, the server will create one for you and set that as your identity.
...
app.get('/set_cookie', function (request, response) { //we're using this to send a cookie with your name data when this route is called
    response.cookie('myname', 'Joey Gomes', {maxAge: 5*1000}).send('cookie set'); //We're taking the response, then setting cookie identifier (myname) and giving it data (Joey Gomes). Then we will send a response back w/ message 'cookie sent'
